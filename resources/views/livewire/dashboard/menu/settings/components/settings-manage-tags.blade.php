...
<div>
   ...
   <form wire:submit='store' class="row align-items-end pt-2">
      ...
      <div class="col-5 text-center">
         ...
         {{-- label --}}
         <div>
            <label class="form-label upload--wrap" data-bs-toggle="tooltip" data-bss-tooltip="" for="tag--file-1"
               title="Click To Upload">

               {{-- file --}}
               <input class="form-control d-none file--input" id="tag--file-1" data-preview="tag--preview-1" type="file"
                  required wire:model='instance.imageFile' />

               {{-- preview --}}
               <img class="inventory--image-frame" id="tag--preview-1" src="{{ url('assets/img/placeholder.png') }}"
                  wire:ignore />
         ...
         <div class="table-responsive memoir--table w-100">
            <table class="table table-bordered" id="memoir--table">
               ...
               <tbody>
                  ...
                  {{-- loop - tags --}}
                  @foreach ($tags as $tag)
                  <livewire:dashboard.menu.settings.components.settings-view-tag :id='$tag->id' key='{{ $tag->id }}' />
                  @endforeach
                  {{-- end loop --}}
