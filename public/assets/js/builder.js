...
window.addEventListener("no-events", (event) => {
    $(document).ready(function () {
        targetClass = event.detail.class;
        targetDelay = event.detail.delay;

        $(targetClass).addClass("no-events-loading");

        setTimeout(function () {
            $(targetClass).removeClass("no-events-loading");
        }, targetDelay);
    });
});
